...
For each configuration one hyper-parameter is optimized at a time,
following the order
...
This is due to the fact that having to use only one head for all classes it is better to be very confident about classyfing a sample as known.
...
as small variations can cause huge differences in results sequentially optimizing hyper-parameters is a sub-optimal heuristic.
Furthermore,
...
A criticality is represented by the normality score hyper-parameter:
